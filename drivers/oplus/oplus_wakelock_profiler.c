...
#include <oplus/qcom_platform_msmnile.h>
...
static int get_platform(void)
{
	...
	if (strstr(comp_str, PLATFORM_MSMNILE)) {
		platform_id = MSMNILE;
		comm_all_modules = (struct wakeup_count_desc_t **)all_modules_msmnile;
		modem_report_exchange = modem_report_exchange_msmnile;
